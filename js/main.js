...
function defUpperStr(text) {
  upperText = text || 'Default text';
  return  upperText.toUpperCase();
}

console.log(defUpperStr('Саша'));
 ...
 */
 ...
 function ageClassification(n) {
   var ageStr = (n < 0) ? null : (n >= 0 && n <= 24) ? 'детский возраст' : (n > 24 && n <= 44) ? 'молодой возраст' : (n > 44 && n <= 65) ? 'средний возраст' : (n > 65 && n <= 75) ? 'пожилой возраст' : (n > 75 && n <= 90) ? 'старческий возраст' : (n > 90 && n <= 122) ? 'долгожители' : null;
   return ageStr
 }
 ...
 */
 ...
 function mainFunc(a, b, callback) { 
  ...
  console.log(mainFunc(1, 100, cbRandom)); 
  console.log(mainFunc(3, 4, cbPow)); 
  console.log(mainFunc(1, 100, cbAdd)); 
